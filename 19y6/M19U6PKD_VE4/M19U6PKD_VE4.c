...
//with Int comparator=off 
   ...
   unsigned char CtMaslo;
    ...
    unsigned char RegimTransmit;
    unsigned char CtEnableTransmit;
...
void	ResetErrorLinkRS(void)
{
	...
	void InitUsart(void)__attribute__ ((section (".bootloader")));			
void	InitUsart(void)
	{	
   ...
   PORTD &=0xf7;//Out Transmit=off
	...
	void TransmitUsart(void)__attribute__ ((section (".bootloader")));			
void	TransmitUsart(void)
   ...
   _WDR();
	...
	void LinkRS(void)__attribute__ ((section (".bootloader")));				  
void	LinkRS(void)
	{
...
//if DDRX,X=1 PORTX,X=OUT
    ...
    PORTD=0xff;
	RegTransmitRS[7]=pgm_read_word_near(0x1bc0);
//INIT TWI;
    TWBR=0x7f;//F TWI
...
//	++TestRS;
	...
	--CtPZA;
 	...
 	SIGNAL(SIG_UART_RECV)__attribute__ ((section (".bootloader")));
