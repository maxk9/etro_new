/* TWI-Master*/
	...
	const unsigned int UMinProt=180;
...
//3	off from out
	...
	else if((RegSWork & 0x1)&&(NumberRegim==2))//Gen
			{
	if(R0 & 1)
	R0 &=0x6e;
...
//	RegTransmit[4][1] |=2;//bit On 
	...
	if(NumberRegim>=4)//Rezerv
		{
	if(RegimClearError & 1)//Set=Norma
			{
	CtTime[0]=CtTime0;
...
//	ASSR=0;
	...
	UNomConst=pgm_read_word_near(0x1bc0);
	if((UNomConst>UNomMax)||(UNomConst<UNomMin))
	UNomConst=230;
	UNom=UNomConst;


	KoefU=pgm_read_word_near(0x1ffe);
	if((KoefU<220)||(KoefU>270))	
	KoefU=241;
...
//	StatizmVozb=0xa;//avton,vozb=off
     ...
     while(1)
    {
     ...
     _WDR();
	 if((!(RomReceive[1][1]& 6))&&(!(RomReceive[2][1]&6)))
	ProgramPage0();
...
//	Period[R2]=0xf0ff;
	...
	CtOverLow[R2]=2;
 	...
 	SIGNAL(SIG_ADC)__attribute__ ((section (".bootloader")));
 SIGNAL(SIG_ADC)
	{
   ...
   if(RegimClearError & (1<<R0))
	...
	SIGNAL(SIG_OVERFLOW2)__attribute__ ((section (".bootloader")));
