...
/*	void	ControlRegim(void)
 ...
 void	ClearErrorFar(void)
 {
	...
	if((URef<220)||(URef>270))	
	URef=241;

	R0=pgm_read_word_near(0x1ffc);//UMax
	if(R0>630)
	R0=600;
	if(R0<570)
	R0=600;
	UMax=R0;
	R0=pgm_read_word_near(0x1ffa);//UMin
	if(R0>250)
	R0=200;
	if(R0<150)
	R0=200;
	...
	R0=pgm_read_word_near(0x1ff8);//IMax
	if(R0>750)
	R0=710;
	if(R0<650)
	R0=710;
...
//INIT TWI;
	...
	while(CtStart--)	_WDR();
//Init capture1
    TCCR1B=0x81;//0.125mkc,falling edge
