...
volatile    unsigned char PORTDD;
    ...
    unsigned char RomReceiveRSOld[7][3];
 ...
 unsigned char WaitTWCR(void)
   ...
   _WDR();
	...
	R0=RomReceiveRS[2][1];
	R1=RomReceiveRS[2][2];
	if(RomReceiveRS[1][1]==RomReceiveRSOld[1][1])
	...
	void	LinkRS(void)
{
   ...
   if(!CtReceiveRS)
	...
	if(R0==ControlReceiveRS)
				{
	for(R4=1;R4<=6;++R4)
	RomReceiveRSOld[R4][NumberReceiveRS]=RamReceiveRS[R4];
