...
//	xTaskWoken = pdFALSE;
	...
	else
	{
		...
		ADC_Arr[cnt_ch] = AdTempResult>>4;//8bit
...
//==================================================================================
void Timer1_IRQHandler(void)//3.1ms
{
...
//	static portBASE_TYPE xTaskWoken;
	...
	//if(cnt_sec > 99)
	{
		...
		{
			if(StatusMU.SPT_Time)
				--StatusMU.SPT_Time;
		}
		else
			StatusMU.SPT_Time=30;
		//ChangeCtTime();
		//ChangeCtError();
...
//Timer1
	...
	sTIM_CntInit.TIMER_Period                   = 200;//3.1 ms
...
void vIndikatorTask( void *pvParameters )
{
	...
	uint8_t cnt_mu = 0, tmp = 0,
			i = 0;
	
	for( ;; )
	{
		if( StatusMU.RegSW&0x80 )
			LED_PJD_ON;
		else
			LED_PJD_OFF;
...
{
 if( StatusMU.RegSW & 0x20 )//sel +
			{
				cnt_mu=0;
				if( StatusMU.CurrPosLCD < 9 )
					++StatusMU.CurrPosLCD;
				else 
					StatusMU.CurrPosLCD = 0;
			}
			
			if( StatusMU.RegSW & 0x10 )//sel -
			{
				cnt_mu=0;
				if( StatusMU.CurrPosLCD )
					--StatusMU.CurrPosLCD;
				else 
					StatusMU.CurrPosLCD = 9;
			}
			
			taskENTER_CRITICAL();
			show_led_data( StatusMU.CurrPosLCD );
			taskEXIT_CRITICAL();
}		
else
			{
		...
		if( DataUsart.count >= (PDG_FRAME_SIZE) )
		{
			...
			if( start )
			{
				...
				if( read_rx_data( rx_data ) == true )
				{
					...
					byte[0] = ( uint8_t ) StatusMU.RegSW;
					byte[1] = ( uint8_t ) (StatusMU.RegSW>>8);
					byte[2] = ( uint8_t ) StatusMU.ADCF;
					byte[3] = ( uint8_t ) StatusMU.ADCU;
					byte[4] = ( uint8_t ) StatusMU.CurrPosLCD;
					
					taskEXIT_CRITICAL();
					
					sum = 1 + byte[0] + byte[1] + byte[2] + byte[3] + byte[4] + 0x55;
					//sum = 2 + 3 + 4 + 5 + 0x55;
					sum = 255 - sum;
					++sum;	
...
//			putchar( '\n' );
		...
		if(!PORT_ReadInputDataBit(MDR_PORTF,PORT_Pin_5))
			RegKn |= 0x80;
		else
			RegKn &= 0xff7f;
