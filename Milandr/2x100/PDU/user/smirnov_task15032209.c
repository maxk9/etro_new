...
extern status_packet PKDU_Status;
...
//	Indicator1((uint8_t)PKDU_Status.DG_Cmd);
	Indicator1(CtUsart[0]);//MDR_TIMER1->CNT>>8);//(uint8_t)PKDU_Status.DG_Cmd);
	Indicator2(TestTimer2);//(uint8_t)PKDU_Status.DG_Fn);	
...
//	static unsigned int R2;
	...
	RegTransmitRS1[38] = 1;//NumberBlock1;
	R1=RegS>>8;
