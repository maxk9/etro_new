/* TWI-Master*/
 ...
 //   const unsigned    int      CtError0[11]={150,250,150,150,250,250,250,150,100,200,200};
   const unsigned    int      CtError0[11]={300,500,300,300,500,1200,1200,300,200,200,200};
...
/*	if(RomReceiveRS[6][3] & 0x80)//Error from SHU,
	...
	if(RomReceiveRS[6][2] >=125)//T>=75
		RegimLuk |=1;//LR=on L1
	else if(RomReceiveRS[6][2] <=110)//T<=60
	...
	else if(RomReceiveRS[6][3] & 0x40)//Error from SHU,
		RegimLuk |=0x8;//Revun=on
	else if(Error & 0x76)//ErrorLuk,RS,Link
		RegimLuk |=0x8;//Revun=on
	else if(RomReceive[5][1] & 0x1e)//ErrorLuk D3
		RegimLuk |=0x8;//Revun=on

	else if(RomReceiveRS[4][2])//Error from MSHU,
		RegimLuk |=0x8;//Revun=on
	else if(RomReceiveRS[5][2])//Error from MSHU,
		RegimLuk |=0x8;//Revun=on
	...
	void LinkRS(void)
{
   ...
   unsigned char ReadEeprom(unsigned char Arg6)
	...
	PORTB |= 0x8;//rele open OFF
	
	if(RegS & 0x2)//Luk3=close
		PORTB |= 0x10;//rele close - OFF	 	
	else 
		if(CtTimeOpen)
			PORTB |= 0x10;//rele close - OFF
		else 
...
//Init capture1
    TCCR1B=0x81;//0.5mkc,falling edge
...
SIGNAL(SIG_ADC)
{
 ...
 SIGNAL(SIG_OVERFLOW1)/*26.2 Mc*/	//8.1 ms osc 8MHz
{
