...
//with Int comparator=off 
	...
	unsigned char NumberRegimOld;
	unsigned char NumberRegimTransmit;
...
*/
    ...
    volatile unsigned int CtStart;
 	...
 	volatile unsigned    int	CtError[12];
 	const unsigned    int CtError0[12]={100,100,30000,60,
										300,100,1,1000,100,100,100,100};
...
//	if(!Error)
			{
	if((RomReceive[4] & 1)&&(!(RegStop))&&(!(RegS & 0x20))&&(TOG>=58))
	RegimWork |=2;
			}
		}
	else if(RegimWork & 1)
...
void	SetRegimError(void)
	...
	RegimError &=0xfffe;
	if((RegS & 0x20)||(RomReceive[5] & 0x10))//KSet=on
		{
	RegimError &=0xffe7;
	Error &=0xffe7;
	CtError[3]=CtError0[3];
	CtError[4]=CtError0[4];
		}
	else
		{
	if(!(RegS1 & 0x40))//RIz1
...
//	NumberRegim=1;
	...
	TestLamp =30;
		...
		{
	PORTC &=0xfb;
	TestLamp =Error & 0xff;//40;
		}
//	else if(TOG<87)
//	PORTC &=0xfb;
	else if(RomReceiveRS[1][0]& 0x60)
		{
	PORTC &=0xfb;
	TestLamp =50;
		}
	else if(RomReceiveRS[3][0]& 0x1e)
		{
	PORTC &=0xfb;
	TestLamp =60;
		}
	...
	else if(RegS & 0x20)//KSet==on
		{
	PORTB &=0xf7;

		}
//	else if((!(RegS & 0x20)) &&(NumberRegim==1))//KSet==off
//	PORTB &=0xf7;
...
//	ControlAvarStop();
	...
	if(PORTC & 8)
	{
	NumberRegimOld=NumberRegim;
	ChangeNumberRegim();
	if(NumberRegimOld==NumberRegim)
	NumberRegimTransmit=NumberRegim;
	}
	ChangeNNom();
	AccountNDiz();
	LoadRegTransmitRS();
...
//INIT TWI;
 ...
 SIGNAL(SIG_OVERFLOW1)/*32.7 Mc*/
    {
	static unsigned char CtDelTim1=3;
 	unsigned char R0;
