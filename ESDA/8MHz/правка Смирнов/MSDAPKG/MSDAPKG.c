/* TWI-Master*/
	...
	unsigned char NumberRegimTemp;
	unsigned char CtBad[3];
	const unsigned int UMaxProt=270;
	const unsigned int UMinProt=196;

	const unsigned int UNomMax=250;
	const unsigned int UNomMin=170;

	unsigned char TestStop1;
	unsigned char TestStop2;
	unsigned char TestStop3;
	unsigned char TestStop;
	unsigned char TestTWI;
	unsigned char StatusKNull;
   unsigned    int    KoefU;
...
volatile	unsigned char CtEnableKn;
    ...
    {120,120,120,120,50,50,50,120,15,15,120,50,50};
...
//3	off from out
	...
	else if(Error & 0x88c)//UGen no norma,no on/off
			{
	R0 &=0xee;	
	++TestStop2;
			}
	else if((R0 & 0x40)&&((NumberRegim>=3)))
			{
	...
	else if((StatusKontaktorGen & 0x10)&&(UGen>195))
	KGenOn();
	...
	if((Error & 0x43)||(RomReceive[6][1] & 0x20))// set no norma, error on/off,RBP
...
*/
	...
	R0=RomReceive[6][1] & 0x7;
	if(R0==NumberRegimTemp)
//	if(!(RomReceive[1][0] & 6))
	NumberRegim=NumberRegimTemp;
	NumberRegimTemp=R0;
//	else
//	NumberRegim=1;
	}
...

{
	...
	if(!(R0 & 4))//KSet=off
	StatusKontaktorSet &=0xdf;//off after || =off
...
//	RegimTime=0;
     ...
     while(1)
    {
     ...
     _WDR();
	 if((UGen<50)||(NumberRegim==1))
	 	{
	CtError[10]=CtError0[10];

		}

	if(RegTransmit[4][1] & 0x10)
	++TestStop;
