...
//	if(!Error)
			{
	if((RomReceive[1] & 1)&&(!(RegStop)))//Pusk
	RegimWork |=2;
			}
		}
	else if(RegimWork & 1)
...
void LoadRegTransmitRS(void)
	...
	R0 |=2;
	if((RomReceiveRS[6][R1]<=115)||(NDiz<400))			
	R0 |=0x20;
...
void	ControlPodogrev(void)
	...
	unsigned char R1;
	R1=NumberBlok-1;
	if(RomReceiveRS[6][R1]>=105)
	PORTB |=1;
	else if(RomReceiveRS[6][R1]<=97)
	PORTB &=0xfe;
