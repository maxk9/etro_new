...
/*	if(RomReceiveRS[1][4] & 1)//AvariaPCH2
	...
	R1 |=0x10;
	if(RomReceiveRS[5][2] & 0x20)//UOg
	R1 |=0x20;
	if(RomReceiveRS[5][5] & 0x20)//UOg
	R1 |=0x40;
	RegTransmit[4][1]=R1;
	...
	void	LinkRS(void)
{
   ...
   if(!CtReceiveRS)
	...
	Error=0;
	ErrorLuk=0;
	RomReceiveRS[5][6] &=0xfb;
	RomReceiveRS[5][3] &=0xfb;
...
*/
	...
	R0|=0x10;		
	if((RomReceive[4][2] & 0xfa)||(RomReceive[5][2] & 0x1f))//G1 Error
	R0|=0x20;	
	if((RomReceive[4][2] & 0x66)||(RomReceive[5][2] & 0xe0)||(RomReceive[6][2] & 0x7))//G2 Error
	R0|=0x40;
	if(RomReceive[6][1] & 8)//FVU
