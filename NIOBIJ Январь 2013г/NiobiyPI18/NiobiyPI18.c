...
//===================================================
const char Niobiy[] PROGMEM={"   ÿ” Õ»Œ¡»…    "};
...
{
	if((TWSR & 0xf8)==0x60)//Adr Slave Receive //Own SLA+W has been received; ACK has been returned
	{						//Slave Receiver Mode
		//	++TestTwi;
		CtReceive=10;
	}
	else if((TWSR & 0xf8)==0x80)//Data Slave Receive 	//Previously addressed with own
	...
	else if((TWSR & 0xf8)==0xA8)//Adr Slave Transmit //Slave Transmitter Mode
	...
	else if((TWSR & 0xf8)==0xb8)//Data Slave Transmit //READ DATA rw=1
...
void ReadKn(void)
{
	...
	R0=RegSTemp;//Temp;
	if(bit_is_clear(PINB,PB2))//nomer shkafa
		R0 |=0x1;
	else
		R0 &=0xfe;
...
void IndNiobiy(void)
{
	...
	{		
		R0=500;
		while(R0--)         _WDR(); 
		R3=pgm_read_byte_near(&Niobiy[R1]);
...
int main(void)
{
	...
	CtInd=50;
		...
		if(TWCR & 0x80) //TWI Interrupt Flag
		{
			++TestTWI;
