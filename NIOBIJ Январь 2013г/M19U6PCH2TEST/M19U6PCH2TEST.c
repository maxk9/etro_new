...
//	++TestTwi;    
//    for(R0=1;R0<=10;++R0)
    RegTransmit[1]=RegimTest;//2;//RegimWork;//UC;
...
/*    RegTransmit[7]=RegTransmitUsart[7];
	...
	else if(RegimTest==10)
		{

	PORTC &=0xfb;
	PORTC &=0xef;
		}
	else if(RegimTest==11)
		{
	...
	else if(RegimTest==12)
		{

	PORTC &=0xfb;
	PORTA &=0x7f;
		}
	else if(RegimTest==13)
		{
    ...
    void ChangeRegimTest(void)
{
    ...
    void IndicatorVar(unsigned char R0)
{
	PORTB |=1;
	PORTA |=0x1f;
	PORTC |=0xc0;
...
//WORK	
	...
	ChangeRegimTest();
	if(RegimTest<=7)
	IndicatorVar(RomReceive[RegimTest]);
	else if(RegimTest<=10)
	IndicatorVar(RomReceive[1]);
