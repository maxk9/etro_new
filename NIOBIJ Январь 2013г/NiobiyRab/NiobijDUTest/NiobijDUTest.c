...
//	if(RegSegmentOn[R1] & (1<<R0))
   ...
   void SetRegSegmentOn (void)
	...
	if(RomReceiveRS[2][0] & 0x40)// Pogar
	RegSegmentOn[3] |=0x4;
	else
	RegSegmentOn[3] &=0xfb;
   ...
   void SetRegSegmentMig (void)
	...
	if(RomReceiveRS[3][0] & 0x1)// Error DG1
	RegSegmentMig[2] |=0x20;
	else
	RegSegmentMig[2] &=0xdf;

	if(RomReceiveRS[3][0] & 0x2)// Error DG2
	RegSegmentMig[3] |=0x1;
	else
	RegSegmentMig[3] &=0xfe;

	if(RomReceiveRS[3][0] & 0x4)// Error PCH1
	RegSegmentMig[1] |=0x1;
	else
	RegSegmentMig[1] &=0xfe;

	if(RomReceiveRS[3][0] & 0x8)// Error PCH2
	RegSegmentMig[1] |=0x10;
	else
	RegSegmentMig[1] &=0xef;

	if(RomReceiveRS[3][0] & 0x20)// Izol
	RegSegmentMig[2] |=0x4;
	else
