/* TWI-Master*/
    ...
    const unsigned    int      CtErrorLuk0=1000;
...
*/
	...
	if(RomReceiveRS[5][6] & 2)//T>=75
	RegTransmit[1][2] |=0x40;//LR2=on
	else if(RomReceiveRS[5][6] & 0x20)//T<=70
	RegTransmit[1][2] &=0xbf;//LR2=off
	if(RomReceiveRS[5][3] & 2)//T>=75
	RegTransmit[1][2] |=0x10;//LR1=on
	else if(RomReceiveRS[5][3] & 0x20)//T<=70
	RegTransmit[1][2] &=0xef;//LR1=off	
			
	if(RomReceiveRS[5][6] & 1)//SHU2 N>400
		{
	RegTransmit[1][2] |=0x80;//LG2=on
	RegTransmit[2][2] |=0x1;//LG2 Osn Prit=on
	if(RegS & 0x2)//T>35
	RegTransmit[1][2] |=0x1;//LG2 Dop Prit=on
	else if(!(RegS & 4))
	RegTransmit[1][2] &=0xfe;//LG2 Dop Prit=off
		}
	else
		{
	RegTransmit[1][2] &=0x7f;//LG2=close
	RegTransmit[2][2] &=0xfe;//LG2 Osn Prit=off
	RegTransmit[1][2] &=0xfe;//LG2 Dop Prit=off
		}
	if(RomReceiveRS[5][3] & 1)//SHU1 N>400
		{
	RegTransmit[1][2] |=0x20;//LG1=on
	RegTransmit[1][2] |=0x8;//LG1 Osn Prit=on
	if(RomReceive[4][1] & 0x10)//T>35
	RegTransmit[1][2] |=0x2;//LG1 Dop Prit=on
	else if(!(RegS & 1))
	RegTransmit[1][2] &=0xfd;//LG1 Dop Prit=off
		}
	else
		{
	RegTransmit[1][2] &=0xdf;//LG1=close
	RegTransmit[1][2] &=0xf7;//LG1 Osn Prit=off
	RegTransmit[1][2] &=0xfd;//LG1 Dop Prit=off
		}
	if((TM>100)&&(R0))
	RegTransmit[1][2] |=4;//Luk vent=on
	else if((TM<85)||(!R0))
	RegTransmit[1][2] &=0xfb;//Luk vent=off
	...
	if(RomReceiveRS[3][5]>=2)
	R0 +=2;
	if(!(RomReceive[1][2] & 0x10))//Luk vent no open
	PORTB |=4;
	...
	R0|=0x10;		
	if((RomReceive[4][2] & 0x78)||(RomReceive[5][2] & 0x1f))//G1 Error
	R0|=0x20;	
	if((RomReceive[4][2] & 0x66)||(RomReceive[5][2] & 0xe0)||(RomReceive[6][2] & 0x7))//G2 Error
	R0|=0x40;
	if(RomReceive[6][1] & 8)//FVU
    ...
    RegTransmit[1][0]=2;//2;//CtErrorRS;//39;    
	RegTransmit[2][0]=R0;
	R0=0;
	if(R1 & 6)
		{
...
/*	void LoadIndicator(void)
    ...
    RegTransmit[2][0]=RomReceive[4][2];//CtErrorLink[3];//4;
    RegTransmit[3][0]=RomReceive[5][2];//CtErrorLink[4];//5;
	RegTransmit[4][0]=RomReceive[6][2];//AdResult>>2;//TM;//4;
    RegTransmit[5][0]=RomReceive[3][2];//TestAd[0];//TestTwi[0];
    RegTransmit[6][0]=RegTransmit[2][1];//omReceiveRS[4][3];//TestAd[0]>>8;//Twi[1];
